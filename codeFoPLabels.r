...
# CKNote: Why above table bmi the same for all ses?
...
system.time(
  tmp <- foreach(sim = 1:montecarlo, .combine = 'rbind', .packages=c("data.table","fst","gamlss")) %dopar% {
    ...
    #TO DELETE ASAP, JUST A SAVING FOR NOW
    # saveRDS(sp, "./todeleteasap.rds")
    # sp <- readRDS("./todeleteasap.rds")
    ...
    ###BECKY SEND AN EMAIL TO THE AUTHOR TO HAVE THE CONFIDENCE INTERVAL, NEED TO BE ADDED ASAP
    #RE - CIs imputed
